<!-- top Products -->
<div class="ads-grid py-sm-5 py-4" style="margin-top:-70px;">
		<div class="container py-xl-4 py-lg-2">
			<!-- tittle heading -->
            <h3 class="tittle-w3l text-center mb-lg-5 mb-sm-4 mb-3">Danh sách các ô đỗ</h3></h3>
			<!-- //tittle heading -->
			<div class="row">
				<!-- product left -->
				<div class="agileinfo-ads-display col-lg-9">
					<div class="wrapper">
					<?php
						$sql_cate_home = mysqli_query($mysqli,"select * from loaiodo order by maloaio");
						while($row_cate_home=mysqli_fetch_array($sql_cate_home )){
							$id_dm=$row_cate_home["maloaio"];
					?>
					<!-- first section -->
					<div class="product-sec1 px-sm-4 px-3 py-sm-5  py-3 mb-4">
						<h3 class="heading-tittle text-center font-italic"><?php echo $row_cate_home["tenloaio"] ?></h3>
						<div class="row">
						<?php
						$sql_product = mysqli_query($mysqli,"select * from odo where trangthai='1' order by maodo");
						while($row_sanpham=mysqli_fetch_array($sql_product)){
							if($row_sanpham["maloaio"]==$id_dm){
						?> 
							<div class="col-md-4 product-men mt-5">
								<div class="men-pro-item simpleCart_shelfItem">
									<div class="men-thumb-item text-center" style="height: 150px;">
										<img src="images/o.jpg" alt="" style="width: 150px; height: auto;">
									...
									<div class="item-info-product text-center border-top">
										<h4 class="pt-1">
											<a href="?quanly=chitietsp&id=<?php echo $row_sanpham["maodo"] ?>"><?php echo $row_sanpham["tenodo"] ?></a>
										</h4>
										<div class="info-product-price my-2">
											<span class="item_price"><?php echo number_format( $row_cate_home["dongia"])."vnđ" ?>/1 Lượt</span>
										</div>
										<div class="snipcart-details top_brand_home_details item_add single-item hvr-outline-out">
										<form action="?quanly=giohang&id=<?php echo $row_sanpham['maodo'] ?>" method="post">
										<fieldset>
											<input type="hidden" name="tensanpham" value="<?php echo $row_sanpham['tenodo'] ?>" />
											<input type="hidden" name="sp_id" value="<?php echo $row_sanpham['maodo'] ?>" />
											<input type="hidden" name="giasanpham" value="<?php echo $row_cate_home['dongia'] ?>" />
